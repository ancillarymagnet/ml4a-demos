...
function draw() {
  ...
  //Draw the face outline
    ...
    var eyebrowHeight = abs(positions[16][1] - positions[25][1]);
      console.log('height = ' + eyebrowHeight);
    ebHeightReadings.push(eyebrowHeight);
      
    var ebMax = Math.max(...ebHeightReadings);
    var ebMin = Math.min(...ebHeightReadings);
    console.log(ebMax);
    ...
    endShape(CLOSE);
      ...
      var noseDir = positions[62][0] - positions[33][0];
    var leftEyeWidth = max(abs(positions[23][0] - positions[27][0]),
                           abs(positions[23][1] - positions[27][1]));
    var red = map(eyebrowHeight, ebMin, ebMax, 0, 255);
      fill(red, 255-red,255-red,255);
    var leStart = positions[27][0];
    for( i = 0; i < eyeCircles; i++){
        
        ellipse(leStart,positions[27][1], leftEyeWidth);
        leStart += noseDir;
    }
      fill(0,0,0,0);
    ...
    endShape(CLOSE);
      ...
      var rightEyeWidth = max(abs(positions[28][0] - positions[32][0]),
                                 abs(positions[28][1] - positions[32][1]));
    fill(red, 255-red,255-red,255);
    var reStart = positions[32][0];
      for( i = 0; i < eyeCircles; i++){
          ellipse(reStart,positions[32][1], rightEyeWidth);
          reStart += noseDir;
      }
